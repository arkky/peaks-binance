    ...
    async def request_tickers_price(self):
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.base_url+self.ticker_price}") as response:
                ...
                with open(f"tickers/{self.name}_price.json", "w") as f:
                    json.dump(data, f)
                return data
    ...
    async def request_status(self):
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.base_url+self.status}") as response:
                data = await response.json()
                with open("tickers/{self.name}_status.json", "w") as f:
                    json.dump(data, f)
                return data
...
class BinanceExchange(BaseExchange):
    ...
    async def get_tickers_price(self):
        return_list = await asyncio.gather(self.request_tickers_price(), self.request_status())
        data_price, data_status = return_list[0], return_list[1]

        bad_tickers = []
        tickers = {}

        for sample in data_status['symbols']:
            symbol = sample['symbol']
            if sample['status'] != "TRADING":
                if symbol not in bad_tickers:
                    bad_tickers.append(symbol)
                    continue
            elif sample['status'] == "TRADING":
                if symbol not in tickers:
                    tickers[symbol] = {}
        
        for sample in data_price:
            sample_symbol = sample['symbol']
            if sample_symbol not in bad_tickers and sample_symbol in tickers:
                tickers[sample_symbol]['price'] = float(sample['price'])

        with open("tickers/{self.name}_tickers_new.json", "w") as f:
            json.dump(tickers, f)
    ...
    async def get_tickers_price(self):
        ...
        for sample in ticker_prices:
            ...
            if base and base in PAIRS and (float(sample['buy']) / float(sample['sell'])) > self.spread_limit and \
                ...
                PAIRS[base]["prices"][quote] = sample["last"]
    ...
    async def get_tickers_price(self):
        ...
        for sample in ticker_prices:
            ...
            if base and (base in PAIRS) and float(sample['close']) != 0 and (float(sample['buyOne']) / float(sample['sellOne'])) > self.spread_limit\
                ...
                PAIRS[base]["prices"][quote] = sample["close"]
...
class OkxExchange(BaseExchange):
    ...
    async def get_tickers_price(self):
        ...
        # ticker_prices = self.request_tickers_price()['data']
        # status_of_coins = self.request_status()['data']
        # coin_infos = self.request_coin_info()['data']
...
class GateExchange(BaseExchange):
    ...
    async def get_tickers_price(self):
        ...
        for sample in ticker_prices:
            ...
            if quote in self.quotes and (base in PAIRS) and float(sample["last"]) > 0 and \
